...
class RawClaimData():
  ...
  def frequent_claimant_analysis(self, sub_policy=None):
    ...
    self.df['year'] = self.df.policy_start_date.dt.year
    self.df.claimant = self.df.policy_number.str.cat(self.df.year.astype(str), sep='_').str.cat(self.df.claimant, sep='_')
    __dep = self.df
  ...
  def export_database(self):
    self.df.to_csv('raw_claim_database.csv', index=False)

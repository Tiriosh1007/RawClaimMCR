...
class RawClaimData():
        ...
        if len(df_h) > 0:
          for col in date_col_hosp:
            if col in df_h.columns.tolist():
              ...
              df_h[col] = df_h[col].astype(str).apply(pd.to_datetime, format='%Y%m%d')
            else:
              df_h[col] = np.nan
  ...
  def __other_format(self, raw_claim_path, insurer, password=None, policy_start_date=None, client_name=None, region='HK'):
    ...
    df_['benefit_type'].loc[df_['benefit_type'].str.contains('aso', case=False)] = 'ASO'
  ...
  def preprocessing(self, policy_id=None, rejected_claim=True, aso=True, smm=True):
    ...
    if rejected_claim == True:
      ...
      self.df = self.df.loc[(self.df.claim_status != 'R') | ~(self.df.claim_status.str.contains('reject', case=False))]

...
class RawClaimData():
  ...
  def __axa_raw_claim(self, raw_claim_path, password=None, policy_start_date=None, client_name=None, region='HK', col_mapper=None):
    ...
    with open(raw_claim_path, "rb") as file:
      ...
      t_df = pd.concat([df_c[self.col_setup].reset_index(drop=True), df_h[self.col_setup].reset_index(drop=True), df_d[self.col_setup].reset_index(drop=True)], axis=0, ignore_index=True)

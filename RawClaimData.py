...
class RawClaimData():
  ...
  def frequent_claimant_analysis(self, export=True, sub_policy=None):
    ...
    __freq_pm_df = self.df[['claimant', 'paid_amount']].loc[self.df.benefit.str.contains('PM', case=False)].groupby(['claimant']).sum().rename(columns={'paid_amount': 'Prescribed Medicine (PM)_paid'})
    ...
    print(self.ip_usage.columns.tolist())
    cols = [f.remove('paid_amount.') for f in self.ip_usage.columns.tolist()]

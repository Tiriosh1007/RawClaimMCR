...
class Shortfall():
  ...
  def __bupa_shortfall(self, shortfall_fp):
    ...
    client_name_ = t_df.iloc[1, 0].split(': ')[1].split('     ')[-1]
    policy_no_ = t_df.iloc[2, 0].split(': ')[1].split('     ')[-1]
    start_d_ = pd.to_datetime(t_df.iloc[3, 0].split(': ')[1].split(' ')[-5], format='%Y-%m-%d')
    end_d_ = pd.to_datetime(t_df.iloc[3, 0].split(': ')[1].split(' ')[-1], format='%Y-%m-%d')

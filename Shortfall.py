...
class Shortfall():
  ...
  def __bupa_shortfall(self, shortfall_fp):
    ...
    client_name_ = t_df.iloc[:, 0].loc[t_df.iloc[:, 0].str.contains('Customer', case=False)].split(': ')[1].split('     ')[-1]
    policy_no_ = t_df.iloc[:, 0].loc[t_df.iloc[:, 0].str.contains('Contract', case=False)].split(': ')[1].split('     ')[-1]

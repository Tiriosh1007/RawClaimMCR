...
class Shortfall():
  ...
  def __bupa_shortfall(self, shortfall_fp):
    ...
    client_name_ = t_df.iloc[:, 0].loc[t_df.iloc[:, 0].str.contains('Customer Name:', case=False)].split(': ')[1].split('     ')[-1]
    policy_no_ = t_df.iloc[:, 0].loc[t_df.iloc[:, 0].str.contains('Contract Number:', case=False)].split(': ')[1].split('     ')[-1]
    start_d_ = pd.to_datetime(t_df.iloc[:, 0].loc[t_df.iloc[:, 0].str.contains('Period:', case=False)].split(': ')[1].split(' ')[-5], format='%Y-%m-%d')
    end_d_ = pd.to_datetime(t_df.iloc[:, 0].loc[t_df.iloc[:, 0].str.contains('Period', case=False)].split(': ')[1].split(' ')[-1], format='%Y-%m-%d')
